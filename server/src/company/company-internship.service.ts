...
import type { CreateInternshipInput, UpdateInternshipInput } from "./company-internship.schemas";
...
const assertMajorPostingLimit = async (
  ...
  const internshipCount = await prisma.internship.count({
    where: {
      ...
      ...(excludedInternshipId ? { id: { not: excludedInternshipId } } : {}),
    },
  });
...
export const createCompanyInternship = async (userId: string, input: CreateInternshipInput) => {
