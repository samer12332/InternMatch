...
export const registerStudent = async (input: StudentRegistrationInput) => {
  try {
    ...
    if (isUniqueConstraintError(error)) {
      throw new AppError("An account with this email or national ID already exists", 409);
    }
...
export const getCurrentUser = async (userId: string) => {
  ...
  const hasValidProfileConfiguration =
    (user.role === UserRole.STUDENT && hasStudentProfile && !hasCompanyProfile) ||
    (user.role === UserRole.COMPANY && hasCompanyProfile && !hasStudentProfile) ||
    (user.role === UserRole.ADMIN && !hasStudentProfile && !hasCompanyProfile);
