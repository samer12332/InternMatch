...
const emailSchema = z.string().trim().email("A valid email is required").transform((value) => value.toLowerCase());
const passwordSchema = z.string().min(8, "Password must be at least 8 characters").max(72, "Password must be at most 72 characters");
const requiredText = (field: string, maxLength: number) => z.string().trim().min(1, `${field} is required`).max(maxLength, `${field} is too long`);

export const studentRegistrationSchema = z.object({
  email: emailSchema,
  password: passwordSchema,
  nationalId: z.string().regex(/^\d{14}$/, "National ID must contain exactly 14 digits"),
