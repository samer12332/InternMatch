...
export const replaceStudentWishes = async (userId: string, input: ReplaceWishesInput) =>
  prisma.$transaction(async (transaction) => {
    ...
    if (eligibleInternships.length !== input.internshipIds.length) {
      throw new AppError("One or more internships are unavailable or ineligible", 400);
    }
